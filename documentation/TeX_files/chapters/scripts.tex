...
\section{Generate Card PDF}
\subsection{Overview}
The \texttt{compile\_pdf.py} script is a Python 3 tool designed to create a PDF from a folder of images, arranging them in a 3x3 grid with no gaps, scaled to standard Magic: The Gathering card size (2.5 $\times$ 3.5 inches). It is used in the TTCG project to produce printable card sheets for trading cards, integrating with images generated by \texttt{create\_card.py}. The script supports letter or A4 page sizes and includes progress tracking and styled error messaging for user feedback.

This tool automates the layout of card images into a PDF, ensuring consistent sizing and alignment for printing, with minimal configuration required.

\subsection{Purpose}
The \texttt{compile\_pdf.py} script streamlines the creation of PDF card sheets for TTCG. Common use cases include:
\begin{itemize}
	\item Producing printable sheets of trading cards for playtesting or production.
	\item Organizing card images into a standardized 3x3 grid layout.
	\item Supporting different page sizes for regional printing needs (e.g., letter for US, A4 for Europe).
\end{itemize}
By scaling images to MTG card dimensions and centering the grid, it ensures professional-quality output with robust error handling.

\subsection{Features}
The script provides the following key features:
\begin{itemize}
	\item \textbf{3x3 Grid Layout}: Arranges up to 9 images per page in a 3x3 grid with no gaps between images.
	\item \textbf{MTG Card Sizing}: Scales all images to 2.5 $\times$ 3.5 inches (180 $\times$ 252 points at 72 DPI).
	\item \textbf{Configurable Page Size}: Supports letter (8.5 $\times$ 11 inches) or A4 ($\sim$8.27 $\times$ 11.69 inches) pages.
	\item \textbf{Centered Grid}: Calculates dynamic margins to center the grid on the page.
	\item \textbf{Progress Tracking}: Displays a progress bar using \texttt{tqdm} during image processing.
	\item \textbf{Styled Messaging}: Uses \texttt{ttcg\_tools.output\_text} for error, warning, and success messages.
	\item \textbf{Error Handling}: Validates grid fit on the page and handles invalid images or folders.
	\item \textbf{Image Support}: Processes \texttt{.png}, \texttt{.jpg}, and \texttt{.jpeg} files.
\end{itemize}

\subsection{Usage}
Invoke the script via the command line:
\begin{lstlisting}[style=terminalstyle]
usage: compile_pdf.py [-h] [--input-folder INPUT_FOLDER] [--output-file OUTPUT_FILE] [--page-size {letter,A4}]

Generate a PDF from images in a 3x3 grid with no gaps.

options:
	-h, --help            show this help message and exit
	-i INPUT_FOLDER, --input-folder INPUT_FOLDER
					Folder containing input images. Defaults to ../images/generated_cards
	-o OUTPUT_FILE, --output-file OUTPUT_FILE
					Output PDF file path. Defaults to: ../images/generated_cards/output.pdf
	-s {letter,A4}, --page-size {letter,A4}
					Page size for the PDF (letter or A4)
\end{lstlisting}

\subsection{Example}
With images in \texttt{generated\_cards/} (e.g., \texttt{card1.png}, \texttt{card2.jpg}), running:
\begin{lstlisting}[style=terminalstyle]
	python3 compile_pdf.py -o cards.pdf
\end{lstlisting}
Produces \texttt{cards.pdf} with images in a 3x3 grid, scaled to 2.5 $\times$ 3.5 inches, centered on letter-sized pages.

\subsection{Requirements}
\begin{itemize}
	\item Python 3.x
	\item \texttt{Pillow} for image processing (\texttt{pip install Pillow}).
	\item \texttt{reportlab} for PDF generation (\texttt{pip install reportlab}).
	\item \texttt{tqdm} for progress bars (\texttt{pip install tqdm}).
	\item Custom TTCG modules: \texttt{ttcg\_tools}, \texttt{ttcg\_constants}.
\end{itemize}

\subsection{Limitations}
\begin{itemize}
	\item Images are stretched to MTG size (2.5 $\times$ 3.5 inches); non-MTG aspect ratios may distort.
	\item Assumes all images can be processed; unsupported formats cause errors.
	\item Grid fit validation assumes uniform image sizes.
\end{itemize}