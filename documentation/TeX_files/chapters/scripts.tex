...
\subsection{Overview}
The Python script \texttt{analyze\_card\_stats.py} is a utility designed to analyze card data from a CSV file in the TTCG (Trading Card Template Generator) format and generate visualizations of type, subtype, and effect mention frequencies. Utilizing libraries such as \texttt{csv}, \texttt{matplotlib}, and \texttt{numpy}, the script processes card data to count occurrences of types and subtypes, tracks mentions of these terms in effect texts, and produces bar plots (and an optional 3D plot) to visualize the results. This tool is particularly useful for game designers seeking to understand the distribution and interactions of card attributes in a deck.

\subsection{Purpose}
The primary purpose of \texttt{analyze\_card\_stats.py} is to provide statistical insights into the card collection, enabling:
\begin{itemize}
